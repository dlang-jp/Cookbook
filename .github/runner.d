...
void unitTest(string[] exDubOpts = null)
{
    if (!".cov".exists)
        mkdir(".cov");
    auto opt = ["-a", config.arch, "--compiler", config.compiler, "--cache=local", "--coverage", "--main-file", ".github/ut.d"]
        ~ exDubOpts;
    string[string] env;
    env.addCurlPath();
...
void createReleaseBuild(string[] exDubOpts = null)
{
    string[string] env;
    env.addCurlPath();
    exec(["dub", "build", "-a", config.arch, "-b=unittest-cov", "--compiler", config.compiler, "--cache=local"]
        ~ exDubOpts, null, env);
}
...
void integrationTest(string[] exDubOpts = null)
{
    ...
    // build
    exec(["dub", "build", "-a", config.arch, "-b=cov", "--compiler", config.compiler, "--cache=local"]
        ~ exDubOpts, null, env);
